...
namespace Nop.Services.Common
{
    ...
    public partial class PdfService : IPdfService
    {
        ...
        protected void ComposeBarcodeElement(IContainer container, WarehouseItem warehouseItem, string productName, string attributes)
        {
            var barcodeWriter = new BarcodeWriterPixelData
            {
                Format = BarcodeFormat.CODE_128,
                Options = new EncodingOptions { Height = 45, Width = 240, Margin = 0 }
            };

            var pixelData = barcodeWriter.Write(warehouseItem.Barcode);
            var image = ConvertPixelDataToBarcodeImage(pixelData);

            container
                .Column(column =>
                {
                    column.Item().Image(image);
                    column.Item().AlignCenter().Text($"{warehouseItem.Barcode}").FontSize(9);
                    column.Item().AlignCenter().Text($"{attributes}").FontSize(5);
                });
        }

        protected byte[] ConvertPixelDataToBarcodeImage(PixelData pixelData)
        {
        ...
        /// <summary>
        /// Export a PDF and displays it on the browser
        /// </summary>
        /// <param name="warehouseItems">Warehouse items</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains a pdf file
        /// </returns>
        public byte[] GenerateBarcodePdf(IList<WarehouseItem> warehouseItems, string productName, string attributes)
        {
            QuestPDF.Settings.DocumentLayoutExceptionThreshold = 50_000;
            var document = Document.Create(container =>
            {
                foreach (var barcodeInfo in warehouseItems)
                {
                    container.Page(page =>
                    {
                        page.Size(288, 144); // 4 inches x 2 inches, where 1 inch = 72 points
                        page.Margin(10);
                        page.PageColor(Colors.White);

                        page.Content().AlignCenter().AlignMiddle().Column(column =>
                        {
                            column.Item().Element(e => ComposeBarcodeElement(e, barcodeInfo, productName, attributes));
                        });
                    });
                }
            })
            .WithMetadata(new QuestPDF.Drawing.DocumentMetadata
            {
                Title = "Barcodes"
            });
