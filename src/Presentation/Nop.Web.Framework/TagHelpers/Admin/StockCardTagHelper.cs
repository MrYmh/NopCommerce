...
namespace Nop.Web.Framework.TagHelpers.Admin
{
    ...
    /// "nop-card" tag helper
    /// </summary>
    [HtmlTargetElement("stock-card", Attributes = NAME_ATTRIBUTE_NAME)]
    public partial class StockCardTagHelper : TagHelper
    {
        #region Constants
        ...
        public StockCardTagHelper(IHtmlHelper htmlHelper)
        {
        ...
        //    }
        //    if(context.AllAttributes[HIDE_BLOCK_ATTRIBUTE_NAME_ATTRIBUTE_NAME] != null)
        //    {
        //        card.Attributes.Add("data-hideAttribute", context.AllAttributes[HIDE_BLOCK_ATTRIBUTE_NAME_ATTRIBUTE_NAME]?.Value.ToString());

        //    }
        //    if(context.AllAttributes[IS_HIDE_ATTRIBUTE_NAME] != null)
        //    {
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            // Create card without any collapsible functionality
            var card = new TagBuilder("div");
            card.AddCssClass("card");
            card.Attributes.Add("id", Name);

            // Create card heading and append title and icon
            var cardHeading = new TagBuilder("div");
            cardHeading.AddCssClass("card-header");

            if (context.AllAttributes.ContainsName(CARD_ICON_ATTRIBUTE_NAME))
            {
                var cardIcon = new TagBuilder("i");
                cardIcon.AddCssClass(context.AllAttributes[CARD_ICON_ATTRIBUTE_NAME].Value.ToString());
                cardHeading.InnerHtml.AppendHtml(cardIcon);
            }

            if (context.AllAttributes.ContainsName(TITLE_ATTRIBUTE_NAME))
            {
                cardHeading.InnerHtml.Append($" {context.AllAttributes[TITLE_ATTRIBUTE_NAME].Value}");
            }
            ...
            contentDiv.AddCssClass("card-body"); // You can remove this if not necessary
            contentDiv.InnerHtml.AppendHtml(childContent.GetContent());
            card.InnerHtml.AppendHtml(contentDiv);

            // Setup output
            output.TagName = "div"; // Ensures it outputs as <div> instead of <nop-card>
